...
resource "aws_lambda_function" "backend" {
  ...
  environment {
    variables = {
      ...
      CONTENT_IMPORT_TOKEN      = lookup(local.ssm_values_by_key, "content-import-token", "")
      SCHEDULED_PUBLISH_TOKEN   = lookup(local.ssm_values_by_key, "scheduled-publish-token", "")
      FRONTEND_REVALIDATE_URL   = var.enable_custom_domain ? "https://${var.root_domain}/api/revalidate" : ""
      FRONTEND_REVALIDATE_TOKEN = lookup(local.ssm_values_by_key, "frontend-revalidate-token", "")
      DEVTO_API_KEY             = lookup(local.ssm_values_by_key, "devto-api-key", "")
      LINKEDIN_ACCESS_TOKEN     = lookup(local.ssm_values_by_key, "linkedin-access-token", "")
      LINKEDIN_AUTHOR_URN       = lookup(local.ssm_values_by_key, "linkedin-author-urn", "")
      LINKEDIN_VERSION          = lookup(local.ssm_values_by_key, "linkedin-version", "")
